...
class ADMIN(commands.Cog, description="Administrative Commands"):
    ...
    @app_commands.command(name="test_app", description="Tests slash commands")
    async def testapp(self, interaction: discord.Interaction) -> None:
        await interaction.response.send_message("test")

    @app_commands.default_permissions(administrator=True)
    @app_commands.command(
        name="send", description="Sends a message in a particular channel"
    )
    async def send(
        self,
        ...
        await interaction.response.send_message(
            f"Sent message to {channel}", ephemeral=True, delete_after=5
        )

    @app_commands.default_permissions(administrator=True)
    @app_commands.command(name="reply", description="Replies to a specified message")
    async def reply(
        self, interaction: discord.Interaction, message_id: str, reply: str
    ):
        message_id = int(message_id)
        message = await interaction.channel.fetch_message(message_id)
        await message.reply(reply)
        await interaction.response.send_message(
            f"Replied to message {message_id}", ephemeral=True, delete_after=5
        )

    @app_commands.default_permissions(administrator=True)
    @app_commands.command(
        name="create_problem_threads",
        description="Creates problem threads for a test. Seperate outcomes by spaces.",
    )
    async def create_problem_threads(
        self,
        interaction: discord.Interaction,
        ...
        try:
            ...
            await interaction.response.send_message(
                f"No tag found for {assn_name}", ephemeral=True, delete_after=5
            )
            return
        for problem in problems:
            await forum.create_thread(
                name=f"{assn_name} Problem {problem}",
                auto_archive_duration=1440,
                content="Post solutions here!",
                applied_tags=[tag],
            )
        await interaction.response.send_message(
            f'Created problem threads for {",".join(problems)} for {assn_name}',
            ephemeral=True,
            delete_after=5,
        )
    ...
    async def solved(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"Marked thread {thread} as solved :white_check_mark:"
        )
    ...
    async def unsolved(self, interaction: discord.Interaction):
        ...
        if tag is None:
            await interaction.response.send_message(
                "This thread is not currently marked as solved.",
                ephemeral=True,
                delete_after=5,
            )
            return
        ...
        await interaction.response.send_message(
            f"Marked thread {thread} as unsolved :red_square:"
        )

    @app_commands.default_permissions(manage_roles=True)
    @app_commands.command(
        name="add_color_roles", description="Adds color roles to the server"
    )
    ...
    async def add_color_roles(
        self, interaction: discord.Interaction, palette_choice: app_commands.Choice[str]
    ):
        ...
        for color in palette.__dict__:
            await interaction.guild.create_role(
                name=color.replace("_", " ").capitalize(),
                color=discord.Colour(int(palette[color][1:], base=16)),
            )
        await interaction.guild.create_role(name=COLOR_SEPERATOR)
